...
class NARS(BaseModel):
    ...
    def __init__(self, num_hops, args, hg):
        ...
        self.category = args.category #指定预测的节点类型
        self.dropout = args.dropout
        self.input_dropout = args.input_dropout#防止过拟合的参数
        # self.feats = feats
        ...
        with th.no_grad():
            self.feats = preprocess_features(hg, mps, args, args.device, self.args.category)#通过 preprocess_features 函数预处理图数据，生成邻居聚合特征
...
def preprocess_features(g, mps, args, device, predict):#处理图数据，生成每个关系子图的邻居聚合特征
    """
