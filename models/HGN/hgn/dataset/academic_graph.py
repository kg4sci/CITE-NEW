...
class AcademicDataset(DGLDataset):
    ...
    def process(self):
        ...
        print(self._g)
        print("Number of nodes:", self._g.num_nodes())
        print("Number of edges:", self._g.num_edges())

        # 查看每个节点的类型和边的类型
        print("Node types:", self._g.ntypes)
        print("Edge types:", self._g.canonical_etypes)

        # 打印每个节点类型的数量
        for ntype in self._g.ntypes:
            print(f"Number of nodes in type '{ntype}':", self._g.num_nodes(ntype))

        # 打印每种边类型的数量
        for etype in self._g.canonical_etypes:
            print(f"Number of edges for edge type '{etype}':", self._g.num_edges(etype))
