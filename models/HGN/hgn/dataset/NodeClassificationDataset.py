...
from openhgnn.utils.utils import add_reverse_edges #!!!
import os
...
#这份代码主要定义了多个用于节点分类数据集的基类，目的是加载、处理和分割不同的数据集
########################        add dataset here
...
#是一个python装饰器，将类注册到某个特定的数据集管理系统中
#作用是自动将一个类和指定的数据集名称绑定
class HIN_NodeClassification(NodeClassificationDataset):
    ...
    def load_HIN(self, name_dataset):
        ...
        # g, _ = load_graphs(data_path)
        # g = g[0]
        elif name_dataset == 'my_custom_node_classification':
            dataset = YourDataset(name='your_dataset_name', raw_dir='')
            print("******在hin_nodeclassification下的执行************")
            g = dataset.graph
            category = 'paper'
        ...
        #     print("******在hin_nodeclassification下的执行************")
        #     g = dataset.graph
        #     category = 'paper'  # 假设 'paper' 是你想要操作的节点类型
        #     num_classes = dataset.num_classes
            
        #     # 如果图是异构图，获取特定节点类型的特征
        #     if category in g.ntypes:  # 检查 category 是否是有效的节点类型
        #         print("**********")
        #         category_features = g.nodes[category].data['h']  # 获取该节点类型的特征
        #         self.in_dim = category_features.shape[1]  # 获取特征的维度
        #     else:
        #         print(f"节点类型 {category} 在图中不存在！")
...
@register_dataset('my_custom_node_classification')
class ChemistryDataset(NodeClassificationDataset):
    def __init__(self, *args, **kwargs):
        super(ChemistryDataset, self).__init__(*args, **kwargs)
        self.g = self.load_graph()
        self.category = 'paper'  # 目标节点类型
        # self.category = [self.category]
        # print(type(self.category))
        self.num_classes = 104  # 单标签分类需要每次确定有多少个类别，多标签共有145个分类
        self.has_feature = True  # 是否包含特征
        self.multi_label = False  # 是否为多标签任务
        self.graph = self.get_split()
        print("init***********")
        print("Number of classes:", self.num_classes)
        print(self.g.ntypes)  # 输出图中所有的节点类型
        print(self.g.nodes['paper'].data)  # 确认'paper'节点的特征数据
        print(self.g.nodes['paper'].data['label'])



    def load_graph(self):
        # 这里写将你的本地数据转换为 DGL HeteroGraph 的代码
        import dgl
        import pandas as pd
        import torch

        # 读取节点文件
        paper_nodes = pd.read_csv("/root/autodl-tmp/dgl/1openHGNN-main/openhgnn/dataset/data/chemistry/nodes/128/paper+author_embedding.csv")  # paper_id, title, feature1, feature2, label
        ...
        # org_nodes = pd.read_csv("/root/autodl-tmp/dgl/1openHGNN-main/openhgnn/dataset/data/chemistry/nodes/org_nodes.csv") #先不要
        #子图模式
        # paper_nodes = pd.read_csv("/root/autodl-tmp/dgl/1openHGNN-main/openhgnn/dataset/data/chemistry/zitu/selected_paper.csv")  # paper_id, title, feature1, feature2, label
        # author_nodes = pd.read_csv("/root/autodl-tmp/dgl/1openHGNN-main/openhgnn/dataset/data/chemistry/zitu/selected_author.csv")  # author_id, name, feature1, label
        # journal_nodes = pd.read_csv("/root/autodl-tmp/dgl/1openHGNN-main/openhgnn/dataset/data/chemistry/zitu/selected_journal.csv")  # venue_id, name
        # keywords_nodes = pd.read_csv("/root/autodl-tmp/dgl/1openHGNN-main/openhgnn/dataset/data/chemistry/zitu/selected_keywords.csv")  # keywords_id, word
        ...
        paper_venue_edges = pd.read_csv("/root/autodl-tmp/dgl/1openHGNN-main/openhgnn/dataset/data/chemistry/edges/paper_journal.csv")  # paper_id, venue_id
        ...
        # author_org_edges = pd.read_csv("/root/autodl-tmp/dgl/1openHGNN-main/openhgnn/dataset/data/chemistry/edges/author-org.csv")#先不要
        #子图模式
        # paper_author_edges = pd.read_csv("/root/autodl-tmp/dgl/1openHGNN-main/openhgnn/dataset/data/chemistry/zitu/selected_paper-author.csv")  # paper_id, author_id
        # paper_venue_edges = pd.read_csv("/root/autodl-tmp/dgl/1openHGNN-main/openhgnn/dataset/data/chemistry/zitu/selected_paper-journal.csv")  # paper_id, venue_id
        # paper_keywords_edges = pd.read_csv("/root/autodl-tmp/dgl/1openHGNN-main/openhgnn/dataset/data/chemistry/zitu/selected_paper-keywords.csv")  # paper_id, keywords_id
        # paper_paper_edges = pd.read_csv("/root/autodl-tmp/dgl/1openHGNN-main/openhgnn/dataset/data/chemistry/zitu/selected_paper-paper.csv")  # paper_id, paper_id
        
        

        # 创建 DGL 的 HeteroGraph
        # print(paper_author_edges['paper_id'].dtype)#object
        # print(paper_author_edges['author_id'].dtype)#object
        # print(paper_author_edges['paper_id'].head())
        # print(paper_author_edges['author_id'].head())


        data_dict = {
            ('paper', 'written_by', 'author'): (torch.tensor(paper_author_edges['paper_id'].values),
                                           torch.tensor(paper_author_edges['author_id'].values)),
            ('paper', 'publish_in', 'journal'): (torch.tensor(paper_venue_edges['paper_id'].values),
                                               torch.tensor(paper_venue_edges['journal_id'].values)),
            ('paper', 'cited_by', 'paper'): (torch.tensor(paper_paper_edges['source'].values),
                                               torch.tensor(paper_paper_edges['target'].values)),
            ('paper', 'contain', 'keywords'): (torch.tensor(paper_keywords_edges['paper_id'].values),
                                               torch.tensor(paper_keywords_edges['keywords_id'].values))
            # ('author', 'work_in', 'org'): (torch.tensor(author_org_edges['author_id'].values),
            #                                    torch.tensor(author_org_edges['org_id'].values))
        }
        # print(data_dict)
        ...
        paper_columns = [f'title+keywords_embedding_{i}' for i in range(128)]
        ...
        # 使用iloc选择这些列
        graph.nodes['author'].data['feature'] = torch.tensor(author_nodes[author_columns].values, dtype=torch.float32)
        graph.nodes['paper'].data['feature'] = torch.tensor(paper_nodes[paper_columns].values, dtype=torch.float32)
        graph.nodes['keywords'].data['feature'] = torch.tensor(keywords_nodes[keywords_columns].values, dtype=torch.float32)
        graph.nodes['journal'].data['feature'] = torch.tensor(journal_nodes[journal_columns].values, dtype=torch.float32)
         
        # 添加节点标签
        graph.nodes['paper'].data['label'] = torch.tensor(paper_nodes['label'].values, dtype=torch.long)
        # graph.nodes['author'].data['label'] = torch.tensor(author_nodes['label'].values, dtype=torch.long)

        # 检查图的信息
        # print(graph)
        # print("Unique labels:", torch.unique(self.g.nodes[self.category].data['label']))
        


        return graph
        # return graph, self.category,self.num_classes

    # def get_split(self):
        # # 这里实现训练、验证和测试集的划分
        # num_papers = self.g.num_nodes('paper')
        # num_train = int(num_papers * 0.6)
        # num_val = int(num_papers * 0.2)

        # all_indices = torch.randperm(num_papers)
    ...
    #             valid_idx = train_idx
    #     else:
    #         # 如果数据集有 train_mask, val_mask, test_mask
    ...
    #     self.test_idx = test_idx
    #     return self.train_idx, self.valid_idx, self.test_idx
    def get_split(self, validation=True):#***************更改split的方式
        """
        Randomly splits the dataset into train/validation/test sets with an 80/10/10 ratio.
        The splits are non-overlapping and random.
        """
        if 'train_mask' not in self.g.nodes[self.category].data:
            self.logger.dataset_info("The dataset has no train mask. "
                                    "So split the category nodes randomly. And the ratio of train/valid/test is 80:10:10.")
            
            num_nodes = self.g.number_of_nodes(self.category)
            
            # Split the indices randomly into 80% train, 10% validation, and 10% test
            ...
            self.logger.dataset_info(f"Train size: {len(train_idx)}, Validation size: {len(val_idx)}, Test size: {len(test_idx)}")
        else:
            # If dataset has train_mask, val_mask, test_mask, use them
            train_mask = self.g.nodes[self.category].data['train_mask']
            test_mask = self.g.nodes[self.category].data['test_mask']
            train_idx = th.nonzero(train_mask, as_tuple=False).squeeze()
            test_idx = th.nonzero(test_mask, as_tuple=False).squeeze()
            ...
            if validation:
                if 'val_mask' in self.g.nodes[self.category].data:
                    ...
                    # Randomly split the train set into train/validation with the ratio of 80:10
                    random_int = th.randperm(len(train_idx))
