...
def load_pretrain_embedding_graph(data_dir, pretrained_embedding_type, data=None):
...
def load_pretrain_embedding_hop(data_dir, pretrained_embedding_type, hop, mask, data=None):
    if pretrained_embedding_type == "cite":
        return [data.x[mask]] * (hop + 1)
    elif pretrained_embedding_type == "simteg":
...
def eval_model(args):
    ...
    if args.task in  ["nc", "nd", "nda", "nctext"]:
        ...
        data_path = os.path.join(data_dir, f"CITE.pt") if args.dataset == "cite" else os.path.join(data_dir, f"processed_data.pt")
    ...
    index = None
    if args.template == "ND":
        pretrained_emb = load_pretrain_embedding_graph(data_dir, args.pretrained_embedding_type, data)
        ...
        if args.dataset in SMALL_DATASETS and args.task == "lp":
            ...
            for q in questions:
                ...
                if "lp" in args.task:
                    ...
                    mask[idx] = True
            pretrained_emb = load_pretrain_embedding_hop(data_dir, args.pretrained_embedding_type, args.use_hop, mask, data)
            index = torch.full([n], fill_value=n + 1, dtype=torch.long)
            test_index = torch.arange(mask.sum())
            index[mask] = test_index
        structure_emb = None
