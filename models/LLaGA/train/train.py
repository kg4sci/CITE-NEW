...
class LazySupervisedGraphDataset(Dataset):
    ...
    def __init__(self,
        ...
        for d, dataset in enumerate(self.use_dataset):
            ...
            if data_args.template == "ND":
                pretrained_emb = self.load_pretrain_embedding_graph(data_dir, data_args.pretrained_embedding_type, data)
                ...
                pretrained_emb = self.load_pretrain_embedding_hop(data_dir, data_args.pretrained_embedding_type, data_args.use_hop, data)
                # n = data.num_nodes
    ...
    def load_pretrain_embedding_graph(self, data_dir, pretrained_embedding_type, data=None):
        if pretrained_embedding_type == "cite":
            # embeddings are stored in data.x (128-dim)
            return data.x
        elif pretrained_embedding_type == "simteg":
    ...
    def load_pretrain_embedding_hop(self, data_dir, pretrained_embedding_type, hop, data=None):
        if pretrained_embedding_type == "cite":
            # embeddings are stored in data.x (128-dim), repeat for each hop level
            return [data.x] * (hop + 1)
        elif pretrained_embedding_type == "simteg":
...
def _train():
    ...
    elif data_args.pretrained_embedding_type in ["cite"]:
        model_args.mm_hidden_size = 128
    else:
        raise ValueError
    if data_args.template == "ND":
